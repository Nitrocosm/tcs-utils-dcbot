...
def write_ids_file(
    ...
    # Build emoji dict — most values are Unicode placeholders, digits follow the
    # 0..9 pattern with no colour variants.
    emoji_lines = []
    for key, val in STAGING_EMOJI_MAP.items():
        emoji_lines.append(f"    {key!r}: {val!r},")
    for i, glyph in enumerate(UNICODE_DIGITS):
        emoji_lines.append(f"    {str(i)!r}: {glyph!r},")
    for color in ["b", "g", "p", "r"]:
        for i, glyph in enumerate(UNICODE_DIGITS):
            emoji_lines.append(f"    {(str(i) + color)!r}: {glyph!r},")

    text = dedent(f'''\
        """Staging-server IDs. Generated by scripts/setup_staging.py.

        Mirrors the surface of ids.py with snowflakes from a staging guild.
        Custom emojis are mapped to Unicode placeholders.

        Edit modules/config/__init__.py to switch from ids.py to this file.
        """

        roles = {{
            "admins": {admin_ids!r},
            "new_people": {new_people_ids!r},
            "role_check": {role_check_ids!r},

            "bot": {rids.get("bot", 0)},
            "leader": {rids.get("leader", 0)},

            "in_vc_leader": {rids.get("in_vc_leader", 0)},
            "in_vc_2_leader": {rids.get("in_vc_2_leader", 0)},
            "in_vc_3_leader": {rids.get("in_vc_3_leader", 0)},

            "in_vc": {rids.get("in_vc", 0)},
            "in_vc_2": {rids.get("in_vc_2", 0)},
            "in_vc_3": {rids.get("in_vc_3", 0)},

            "available_leader": {rids.get("available_leader", 0)},
            "available": {rids.get("available", 0)},
            "available_not_in_vc": {rids.get("available_not_in_vc", 0)},
            "available_not_in_vc_2": {rids.get("available_not_in_vc_2", 0)},
            "available_not_in_vc_3": {rids.get("available_not_in_vc_3", 0)},
            "not_available": {rids.get("not_available", 0)},

            "birthday": {rids.get("birthday", 0)},
            "inactive": {rids.get("inactive", 0)},
            "explained_inactive": {rids.get("explained_inactive", 0)},
            "person": {rids.get("person", 0)},
            "newbie": {rids.get("newbie", 0)},
            "warn_1": {rids.get("warn_1", 0)},
            "warn_2": {rids.get("warn_2", 0)},
            "warn_3": {rids.get("warn_3", 0)},
            "mod": {rids.get("mod", 0)},
            "spoiler": {rids.get("spoiler", 0)},
            "verifier": {rids.get("verifier", 0)},
            "alts": {rids.get("alts", 0)},

            "lb_display_top_1": {rids.get("lb_display_top_1", 0)},
            "lb_display_top_2": {rids.get("lb_display_top_2", 0)},
            "lb_display_top_3": {rids.get("lb_display_top_3", 0)},
            "lb_display_not_top": {rids.get("lb_display_not_top", 0)},

            "lb_top_1": {rids.get("lb_top_1", 0)},
            "lb_top_2": {rids.get("lb_top_2", 0)},
            "lb_top_3": {rids.get("lb_top_3", 0)},

            "completion_all_base": {rids.get("completion_all_base", 0)},
            "completion_all_ultimate": {rids.get("completion_all_ultimate", 0)},
        }}
        ...
        channels = {{
            ...
            "availability_reaction": 0,  # custom emoji id — Unicode placeholder used, no snowflake
            ...
            "spoiler_role": {rids.get("spoiler", 0)},  # historically mis-keyed
        ...
        # ── Reaction roles ─────────────────────────────────────────────────────
        REACTION_ROLES = {{
            {mids.get("reaction_roles_spoiler", 0)}: {{
                "⚠️": {rids.get("spoiler", 0)},
            }},
        }}
        ...
        {chr(10).join(f"    {fid}: {tid}," for fid, tid in forum_tag_pairs)}
        }}
        ''')
