...
def write_ids_file(
    ...
    L("")
    L("")
    L("# ── Module-level channel/message IDs (mirrors the production ids.py tail) ──")
    L(f"VERIFICATION_FORUM_ID = {cids.get('VERIFICATION_FORUM_ID', 0)}")
    L(f"RELATIONS_CHANNEL_ID = {cids.get('RELATIONS_CHANNEL_ID', 0)}")
    L(f"WARDROBE_CHANNEL_ID = {cids.get('WARDROBE_CHANNEL_ID', 0)}")
    L("")
    L(f"INTERESTED_MESSAGE_BASE = {mids.get('INTERESTED_MESSAGE_BASE', 0)}")
    L(f"INTERESTED_MESSAGE_STAR = {mids.get('INTERESTED_MESSAGE_STAR', 0)}")
    L(f"INTERESTED_MESSAGE_ULTIMATE = {mids.get('INTERESTED_MESSAGE_ULTIMATE', 0)}")
    L("")
